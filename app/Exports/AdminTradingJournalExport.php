...
class AdminTradingJournalExport implements FromCollection, WithTitle, WithHeadings, WithMapping, WithEvents, ShouldAutoSize, WithStyles
...
public function collection()
    {
        $query = TradingJournal::where('user_id', $this->userId);

        // ✅ Filter by month/year if provided
        if (!empty($this->month)) {
            $query->whereMonth('open_date', $this->month);
        }
        if (!empty($this->year)) {
            $query->whereYear('open_date', $this->year);
        }
...
$drawdownPenalty = abs($drawdownPenalty);
    ...
    [$consistencyPoints, $consistencyGrade] = ($totalTrades >= 1 && is_numeric($stdDeviation)) ? match (true) {
        ...
        default              => [0, 'F'],
            ...
            $grades = collect([$winRateGrade, $rrrGrade, $growthGrade, $drawdownGrade]);
