...
<title>Add Trading Signal | HC Gaming Studio</title>
...
<div class="page-content signal-form-page">
    <div class="container-fluid signal-form-shell">
        <div class="form-hero">
            <div>
                <div class="form-kicker">Create Signal Ticket</div>
                <h3>Add Trading Signal</h3>
                <p>Prepare the broadcast scope, execution levels, rationale, and chart context before publishing to the selected community channels.</p>
            </div>
            <div class="d-flex align-items-end">
                <a href="{{ route('all.trading.signals') }}" class="btn btn-outline-light">
                    <i class="ri-arrow-left-line"></i> Back To Signals
                </a>
            </div>
        </div>

        <form method="POST" action="{{ route('store.trading.signal') }}" id="submitSignal" enctype="multipart/form-data">
            @csrf
                ...
                <div>
                    <div class="signal-panel">
                        <div class="panel-head">
                            <div>
                                <h5>Broadcast Scope</h5>
                                <p>Choose where the signal should be published.</p>
                            </div>
                        </div>
                        <div class="row g-3">
                            <div class="col-lg-6">
                                <label class="form-label required">Send To Community</label>
                                <select name="community_target" class="form-select" required>
                                    <option value="all">All Communities</option>
                                    @foreach($communities as $community)
                                        <option value="{{ $community->id }}" {{ old('community_target') == $community->id ? 'selected' : '' }}>
                                            {{ $community->name }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('community_target') <div class="text-danger small mt-1">{{ $message }}</div> @enderror
                            </div>
                            <div class="col-lg-6">
                                <label class="form-label required">Community Category</label>
                                <select name="community_category" class="form-select" required>
                                    <option value="all">All Categories</option>
                                    @foreach($categories as $category)
                                        <option value="{{ $category }}" {{ old('community_category') == $category ? 'selected' : '' }}>
                                            {{ $category }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('community_category') <div class="text-danger small mt-1">{{ $message }}</div> @enderror
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="signal-panel">
                        <div class="panel-head">
                            <div>
                                ...
                                <p>Define pair, action, entry, protection, and risk profile.</p>
                            </div>
                        </div>
                        <div class="row g-3">
                            <div class="col-lg-4">
                                <label class="form-label required">Trading Pair</label>
                                <input name="trading_pair" class="form-control" type="text" value="{{ old('trading_pair') }}" placeholder="XAU/USD" required>
                            ...
                            <div class="col-lg-4">
                                ...
                                <select name="immediate_action" class="form-select" required>
                                    ...
                                    @foreach(['Buy Now', 'Sell Now', 'Buy Limit', 'Sell Limit', 'Buy Stop', 'Sell Stop'] as $action)
                                        <option value="{{ $action }}" {{ old('immediate_action') == $action ? 'selected' : '' }}>{{ $action }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-lg-4">
                                ...
                                <input name="risk_level" class="form-control" type="text" value="{{ old('risk_level') }}" placeholder="Low / Medium / High">
                                @error('risk_level') <div class="text-danger small mt-1">{{ $message }}</div> @enderror
                            </div>
                            <div class="col-lg-6">
                                <label class="form-label required">Entry Price</label>
                                <input name="entry_price" class="form-control" type="text" value="{{ old('entry_price') }}" placeholder="Entry price" required>
                                @error('entry_price') <div class="text-danger small mt-1">{{ $message }}</div> @enderror
                            </div>
                            <div class="col-lg-6">
                                <label class="form-label required">Stop Loss</label>
                                <input name="stop_loss" class="form-control" type="text" value="{{ old('stop_loss') }}" placeholder="Stop loss" required>
                    ...
                    <div class="signal-panel">
                        <div class="panel-head">
                            <div>
                                ...
                                <p>TP1 and TP2 are required. Additional targets are optional but recommended for staged management.</p>
                            </div>
                        </div>
                        <div class="target-grid">
                            @for ($t = 1; $t <= 10; $t++)
                                <div class="target-field">
                                    ...
                                    <input name="target_{{ $t }}" class="form-control" type="text" placeholder="TP{{ $t }} price" value="{{ old('target_'.$t) }}" {{ $t <= 2 ? 'required' : '' }}>
                    ...
                    <div class="signal-panel">
                        <div class="panel-head">
                            <div>
                                <h5>Rationale And Attachments</h5>
                                <p>Add trade reasons, a TradingView link, disclaimer, and optional chart image.</p>
                            </div>
                        </div>
                        <div class="row g-3">
                            <div class="col-lg-6">
                                <label class="form-label">Trading Reasons</label>
                                <select name="trading_reasons[]" multiple class="form-control">
                                    @foreach ($reasons as $reason)
                                        <option value="{{ $reason->id }}" {{ in_array($reason->id, old('trading_reasons', [])) ? 'selected' : '' }}>
                                            {{ $reason->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-lg-6">
                                ...
                                <input name="link" class="form-control @error('link') is-invalid @enderror" type="url" value="{{ old('link') }}" placeholder="https://www.tradingview.com/x/...">
                                @error('link') <div class="invalid-feedback">{{ $message }}</div> @enderror
                            </div>
                            <div class="col-lg-12">
                                <label class="form-label">Disclaimer</label>
                                <textarea name="disclaimer" class="form-control" rows="3" placeholder="Optional disclaimer">{{ old('disclaimer') }}</textarea>
                                @error('disclaimer') <div class="text-danger small mt-1">{{ $message }}</div> @enderror
                            </div>
                            <div class="col-lg-12">
                                <label class="form-label">Chart Image</label>
                                <input type="file" name="signal_image" class="form-control" accept="image/*">
                                @error('signal_image') <div class="text-danger small mt-1">{{ $message }}</div> @enderror
                                <img id="imagePreview" class="signal-image-preview" src="#" alt="Signal image preview">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <aside class="signal-side">
                    <div class="panel-head">
                        <div>
                            ...
                            <p>Complete the required execution fields before publishing.</p>
                        </div>
                    </div>
                    <div class="side-row">
                        <span>Required Setup</span>
                        <strong>Pair, action, entry, SL, TP1, TP2</strong>
                    </div>
                    <div class="side-row">
                    ...
                    <div class="submit-row">
                        ...
                        <button type="submit" class="btn btn-primary" id="submitbutton">
                            <i class="ri-send-plane-line"></i> Publish Signal
                        </button>
                    </div>
                </aside>
            </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.getElementById('submitSignal').addEventListener('submit', function () {
            document.getElementById('submitbutton').disabled = true;
            document.getElementById('submitbutton').textContent = 'Publishing...';
        });

        const imageInput = document.querySelector('input[name="signal_image"]');
        if (imageInput) {
